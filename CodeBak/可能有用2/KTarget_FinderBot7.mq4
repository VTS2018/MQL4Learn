...
#property strict
//+------------------------------------------------------------------+
//| KTarget_Finder6 和 KTarget_FinderBot7 搭配组合在一起 实现共享大脑的逻辑计算
//+------------------------------------------------------------------+
...
//插入V3引擎的代码配置信息
#include <Config_Engine_v3.mqh>

// [新增全局变量] 主动风控状态记录
datetime g_LastActiveExitTime = 0; // 上次触发主动离场的时间
...
int OnInit()
{
   // --- [步骤 1] 基础映射：从 EA 输入参数赋值 ---
   // 即使不开启智能调优，也要保证基础变量有值 [cite: 8, 9]
   Scan_Range = Indi_Scan_Range;
   Lookahead_Bottom = Indi_Lookahead_Bottom;
   Lookback_Bottom = Indi_Lookback_Bottom;
   Lookahead_Top = Indi_Lookahead_Top;
   Lookback_Top = Indi_Lookback_Top;
   Max_Signal_Lookforward = Indi_Max_Signal_Look;
   Look_LLHH_Candles = Indi_LLHH_Candles;

   // --- [步骤 2] 智能同步：复刻指标的自适应逻辑 ---
   // 🚨 这里的开关 Indi_Smart_Tuning 必须与指标的 Smart_Tuning_Enabled 逻辑一致 [cite: 24]
   if (Indi_Smart_Tuning)
   {
      // 调用共享内核中的调优函数
      ...
      // Print(" [智能同步] EA 已根据周期 ", EnumToString(_Period), " 自动优化内核参数。");
      Print(" [智能同步] EA 已根据周期 ", EnumToString((ENUM_TIMEFRAMES)_Period), " 自动优化内核参数。");
   }
...
void OnTick()
{
   ...
   // =========================================================
   // 🔀 引擎路由 (The Router) - 稍后我们将在这里插入切换开关
   // =========================================================
   ...
   if (Enable_V3_Engine)
   {
      // --- 引擎 A: v3.0 智能引擎 ---
      // 1. 先执行主动风控 (巡逻现有订单)
      if (Enable_Active_Exit)
      {
         ManageOpenTrades_Pro();
      }

      // 2. 再扫描新信号 (寻找开单机会)
      ScanForTradeSignal_v3();
   }
   else
   {
      // 目前暂时只调用旧引擎，确保代码没坏
...
//+------------------------------------------------------------------+
//| OnTick_Legacy_V1
//| 功能：封装 Bot6 原始的信号扫描与决策逻辑 (用于回滚和对比)
//+------------------------------------------------------------------+
void OnTick_Legacy_V1()
{
    // ... 在这里粘贴您从 OnTick 剪切出来的旧代码 ...
    // 从 "4.0 第一阶段：数据准备" 开始
    // 到 "for (int i = 0; i < total_valid_signals; i++)" 循环结束
