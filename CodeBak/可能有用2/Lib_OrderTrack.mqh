...
// ✅ v3 主动风控巡逻队
//+------------------------------------------------------------------+
//| ManageOpenTrades_Pro
//| 功能：v3 主动风控巡逻队 (Active Risk Management)
//+------------------------------------------------------------------+
void ManageOpenTrades_Pro()
{
   // 遍历所有订单
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES) == false) continue;
      if(OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber) continue;
      if(OrderType() > OP_SELL) continue; // 不处理挂单

      // 计算当前浮动盈亏 (点数)
      double profit_pts = 0;
      if (OrderType() == OP_BUY)  profit_pts = (Bid - OrderOpenPrice()) / Point;
      else                        profit_pts = (OrderOpenPrice() - Ask) / Point;

      // 🚨 调用内核的风控检测引擎
      // 注意：CheckSignalInvalidation_Pro 必须在 K_Logic_v3.mqh 中已定义
      int invalid_code = CheckSignalInvalidation_Pro(
            Symbol(), Period(), OrderType(), 
            OrderOpenPrice(), OrderOpenTime(), profit_pts,
            P1_Buffer_Mode, P1_Tolerance, P1_Confirm_Bars,
            Max_Stagnant_Bars, Time_Exit_Min_Profit
      );

      // 如果发现异常
      if (invalid_code != 0)
      {
         string reason = (invalid_code==1)?"假突破(P1回归)":(invalid_code==2)?"时间衰竭(僵尸单)":"动能丧失";
         
         if (Show_Debug_Marks) // A. 影子模式：仅标记
         {
             string obj_name = "InvX_" + IntegerToString(OrderTicket());
             if(ObjectFind(0, obj_name) < 0) {
                ObjectCreate(0, obj_name, OBJ_ARROW, 0, Time[0], (OrderType()==0?Low[0]:High[0]));
                ObjectSetInteger(0, obj_name, OBJPROP_ARROWCODE, 251); // X 符号
                Print("🛡️ [影子风控] 建议离场 #", OrderTicket(), " 原因: ", reason, " 浮盈:", profit_pts);
             }
         }
         else // B. 实战模式：杀伐果断
         {
             g_LastActiveExitTime = TimeCurrent(); // 记录时间用于冷却
             bool res = OrderClose(OrderTicket(), OrderLots(), OrderClosePrice(), 3, clrYellow);
             if(res) Print("🛡️ [主动风控] 强制离场 #", OrderTicket(), " 原因: ", reason);
         }
      }
   }
}