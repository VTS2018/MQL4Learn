...
//+------------------------------------------------------------------+
//| 信号弱势过滤：检查看涨信号的 SL (低点) 是否抬升                 |
//| 返回值：过滤后有效信号的数量。                                   |
//| 参数1 (source_signals): 原始信号列表 (按值复制)                  |
//| 参数2 (&filtered_list): 过滤后的结果数组 (通过引用填充)          |
//+------------------------------------------------------------------+
int FilterWeakBullishSignals(FilteredSignal &source_signals[], FilteredSignal &filtered_list[])
{
    ...
    // 从最旧的信号开始比较。最旧的信号在数组的最后一个元素。
    int last_index = size - 1;
    
    // 3.1 假设最旧的信号（信号 B - 最远离 K[1]）总是有效的，将其加入结果列表
    ArrayResize(filtered_list, 1);
    filtered_list[0] = source_signals[last_index]; 
    
    // 3.2 逆序遍历：从倒数第二个信号开始，向前比较 (信号 A - 更靠近 K[1])
    for (int i = last_index - 1; i >= 0; i--)
    {
        FilteredSignal current_signal = source_signals[i];   // 信号 A (新的，shift 更小)
        FilteredSignal previous_valid = filtered_list[ArraySize(filtered_list) - 1]; // 上一个有效信号 (旧的，shift 更大)
        
        // -------------------------------------------------------------
        // 🚨 过滤规则 (看涨信号弱势判断) 🚨
        // 看涨要求：低点 (SL) 必须抬升。
        // 如果当前信号的止损价 (SL) 小于或等于前一个有效信号的止损价 (SL)，
        // 则表明价格低点被破坏或未抬升，视为无效信号。
        // -------------------------------------------------------------
        
        if (current_signal.stop_loss <= previous_valid.stop_loss)
        {
            // Print("❌ 弱势过滤 (看涨): 信号 @ ", TimeToString(current_signal.signal_time), " 未能抬升低点，过滤掉。");
            continue; // 过滤掉信号 A
        }
    ...
    // 4. 逆转数组：当前 filtered_list 是从旧到新排列的，将其逆转为从新到旧 (K[1] 的信号排在最前)
    ArrayReverse_MQL4(filtered_list); 
...
int FilterWeakBearishSignals(FilteredSignal &source_signals[], FilteredSignal &filtered_list[])
{
    ...
    for (int i = last_index - 1; i >= 0; i--)
    {
        ...
        // 🚨 过滤规则 (看跌信号弱势判断) 🚨
        // 看跌要求：高点 (SL) 必须降低。
        // 如果当前信号的止损价 (SL) 大于或等于前一个有效信号的止损价 (SL)，
        // 则表明价格高点被破坏或未降低，视为无效信号。
        // -------------------------------------------------------------
        
        if (current_signal.stop_loss >= previous_valid.stop_loss)
        {
            // Print("❌ 弱势过滤 (看跌): 信号 @ ", TimeToString(current_signal.signal_time), " 未能降低高点，过滤掉。");
            continue; // 过滤掉信号 A
        }
...
void RunTestLogic()
{
    ...
    // 步骤 1：定义数组并收集所有信号
    FilteredSignal bullish_signals[]; 
    FilteredSignal bearish_signals[]; 
    
    CollectAllSignals(bullish_signals, bearish_signals);

    Print("原始收集结果：看涨信号总数=", ArraySize(bullish_signals), 
          " | 看跌信号总数=", ArraySize(bearish_signals));
    /*
    // 步骤 2：对两个列表分别应用弱势过滤
    
    // 2.1 过滤看涨信号 (使用您之前逻辑：SL必须抬升)
    FilteredSignal final_bullish_list[] = FilterWeakBullishSignals(bullish_signals);

    // 2.2 过滤看跌信号 (高点必须降低)
    FilteredSignal final_bearish_list[] = FilterWeakBearishSignals(bearish_signals);
    ...
    */
    ...
    // 2.1 过滤看涨信号
    FilteredSignal final_bullish_list[]; 
    int bullish_count = FilterWeakBullishSignals(bullish_signals, final_bullish_list);

    // 2.2 过滤看跌信号
    FilteredSignal final_bearish_list[];
    int bearish_count = FilterWeakBearishSignals(bearish_signals, final_bearish_list);

    // 步骤 3：打印最终结果并遍历
    
    // 打印看涨结果
    Print("\n--- 过滤后的看涨有效信号列表 (从 K[1] 往左) ---");
    Print("最终看涨信号总数:", ArraySize(final_bullish_list));

    for (int i = 0; i < ArraySize(final_bullish_list); i++)
    {
        FilteredSignal s = final_bullish_list[i];
        Print("看涨信号 #", i + 1, ": Time=", TimeToString(s.signal_time), 
              " | K[", s.shift, "] | Close=", DoubleToString(s.confirmation_close, _Digits), 
              " | SL=", DoubleToString(s.stop_loss, _Digits));
    }

    // 打印看跌结果
    Print("\n--- 过滤后的看跌有效信号列表 (从 K[1] 往左) ---");
    Print("最终看跌信号总数:", ArraySize(final_bearish_list));

    for (int i = 0; i < ArraySize(final_bearish_list); i++)
    {
        FilteredSignal s = final_bearish_list[i];
        Print("看跌信号 #", i + 1, ": Time=", TimeToString(s.signal_time), 
              " | K[", s.shift, "] | Close=", DoubleToString(s.confirmation_close, _Digits), 
              " | SL=", DoubleToString(s.stop_loss, _Digits));
    }
...
//+------------------------------------------------------------------+
//| MQL4 兼容函数：逆转 FilteredSignal 数组元素顺序                 |
//| 注意：MQL4 没有内置 ArrayReverse，我们必须手动实现。             |
//+------------------------------------------------------------------+
void ArrayReverse_MQL4(FilteredSignal &arr[])
{
    int size = ArraySize(arr);
    if (size <= 1) return; // 0 或 1 个元素的数组无需逆转
    
    // 我们只需要迭代到数组大小的一半
    int half_size = size / 2;
    FilteredSignal temp; // 临时存储，用于交换

    for (int i = 0; i < half_size; i++)
    {
        // 交换 arr[i] 和 arr[size - 1 - i]
        temp = arr[i];
        arr[i] = arr[size - 1 - i];
        arr[size - 1 - i] = temp;
    }
...
// ⚠️ 注意：您需要手动调用 RunTestLogic() 函数，通常在 OnStart() 或 OnInit() 中。
// RunTestLogic();
