...
// 定义图表对象名称 (全局常量或宏)
#define TRADE_STATUS_OBJECT_NAME "MaxOrderLimitWarning"
...
void OnTick()
{
   ...
   // Print("--->[KTarget_FinderBot.mq4:264]: current_positions: ", current_positions);
   ...
   if (current_positions >= Max_Open_Orders)
   {
      ...
      // 准备警告信息
      string warning_message = StringFormat(
          "【🛑 风控警告】\n最大持仓限制触发！\n当前持仓: %d / %d (上限)",
          current_positions,
          Max_Open_Orders);

      // ** NEW: 将警告信息输出到图表右下角 **
      DrawTradeStatusInfo(warning_message, TRADE_STATUS_OBJECT_NAME, clrRed);
   ...
   else
   {
      ...
      // ** NEW: 清除图表右下角的警告提示 **
      // 传入空字符串，触发 DrawTradeStatusInfo 函数内的 ObjectDelete 逻辑。
      DrawTradeStatusInfo("", TRADE_STATUS_OBJECT_NAME);
   }
   ...
   if (current_type == OP_SELL)
   {
      ...
      for (int i = 0; i < total_bulls; i++)
      {
         ...
         for (int z = 0; z < g_FiboZonesCount; z++)
         {
            ...
            {
               // ----------------------------------------------------
               // 🚨 NEW: 看跌信号 触碰 看涨锚点阻力区 -> 绘制红色连接线
               // ----------------------------------------------------
               // 对象名称确保唯一性，包含两个 K 线的 shift 索引
               // string obj_name = context_line_prefix + current_shift + "_" + prev.shift;
               // ObjectDelete(0, obj_name); // 删除旧的同名对象

               // DrawTrendLineObject(
               //     obj_name,             // 唯一的对象名称
               //     Red,                  // 颜色：红色（表示压力/阻力）
               //     2,                    // 宽度
               //     Time[current_shift],  // 起点时间 (当前 K 线)
               //     Close[current_shift], // 起点价格 (当前 K 线收盘)
               //     Time[prev.shift],     // 终点时间 (历史锚点)
               //     Close[prev.shift]     // 终点价格 (历史锚点收盘)
               // );
               // ----------------------------------------------------
               ...
               // 1. 🚨 关键修正：使用 K 线时间戳作为对象名称，保证唯一性和持久性
               //    格式: Prefix + 当前K线时间戳字符串 + "_" + 锚点K线时间戳字符串
               string time_str_A = GetBarTimeID(current_shift); // TimeToString(Time[current_shift], TIME_DATE | TIME_SECONDS);
               string time_str_B = GetBarTimeID(prev.shift);    // TimeToString(Time[prev.shift], TIME_DATE | TIME_SECONDS);
               string obj_name = context_line_prefix + time_str_A + "_" + time_str_B;
               // KTBot_445792563_CRel_251210_0510_251209_1630 2025.12.10 05:10:00

               ObjectDelete(0, obj_name); // 删除旧的同名对象 (如果有)

               // 2. 确定坐标
               datetime time_A = Time[current_shift];
               double price_A = Close[current_shift];
               datetime time_B = Time[prev.shift];
               double price_B = Close[prev.shift];
      ...
      for (int i = 0; i < total_bears; i++)
      {
         ...
         for (int z = 0; z < g_FiboZonesCount; z++)
         {
            ...
            if (current_low <= check_high && current_high >= check_low)
            {
               ...
               // ----------------------------------------------------
               // 1. 🚨 关键修正：使用 K 线时间戳作为对象名称，保证唯一性和持久性
               string time_str_A = GetBarTimeID(current_shift);
               string time_str_B = GetBarTimeID(prev.shift);
               string obj_name = context_line_prefix + time_str_A + "_" + time_str_B;
