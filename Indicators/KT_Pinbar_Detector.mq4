...
input double   PinRatio = 2.0;          // Main Shadow/Body Ratio (>=2.0)
input double   NoseRatio = 0.3;         // Opposite Shadow/Body Ratio (<=0.3)
input double   BodyRatio = 0.33;        // Body/Total Range Ratio (<=0.33)
...
input double   AlertRatio = 3.0;        // Alert Quality Threshold (>=3.0)
...
bool IsBullishPinbar(int index)
{
   ...
   // 条件 4：实体位于上方 2/3 区域（可选，更严格）
   double bodyPosition = (MathMin(o, c) - l) / totalRange;
   bool bodyAtTop = bodyPosition >= 0.6; // 实体在上方 60% 以上
...
bool IsBearishPinbar(int index)
{
   ...
   bool bodyAtBottom = bodyPosition >= 0.6; // 实体在下方 60% 以上
